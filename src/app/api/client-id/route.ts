...
/**
 * Solid-OIDC **Client Identifier Document** for Mind Contacts.
 *
 ...
 * so the IdP's "Remember this client" never sticks and the consent screen
 * reappears on every sign-in. Pointing `login({ clientId })` at this stable,
 * dereferenceable document gives the app one durable identity.
 *
 ...
 * URL, so the client only uses this on non-local origins — see `startLogin`
 * in ConnectForm. This route serves a static document; it persists nothing.
 */
