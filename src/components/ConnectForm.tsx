...
const CLIENT_NAME = "Mind Contacts";
const TAGLINE = "People you know, in your pod.";
 ...
 * for consent on every load. Gated to non-localhost origins: a containerised
 * dev IdP can't dereference a `http://localhost:3130/...` client id document,
 * so dev keeps the dynamic-registration path.
 */
    ...
    <>
      <MindLoginCard
        ...
        onLogin={async ({ issuer }) => {
          ...
          // Fall back to /contacts only if a deep link wasn't already
          // remembered by the signed-out screen the user came from.
          rememberReturnToDefault("/contacts");
