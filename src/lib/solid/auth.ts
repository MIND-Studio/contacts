...
const RETURN_TO_KEY = "mind-contacts:return-to";
 ...
 * an infinite /login/callback ↔ /contacts loop (learned in mind-drive), and
 * even in the happy path it round-trips through the IdP and discards the deep
 * link. The price is that a hard refresh lands on the signed-out prompt; we
 * soften that by remembering the attempted path (see `rememberSignedOutPath`)
 * so reconnecting returns there.
 */
...
/**
 * True when Contacts is running inside another origin's frame (e.g. the Mind
 ...
 * (header account chip + page body + callback page), so memoizing the call to
 * a module-level promise guarantees the redirect is handled exactly once per
 * page load no matter how many components ask for the session.
 *
 * HARD RULE: this is the ONLY call site of `handleIncomingRedirect` in this
...
export async function ensureSession(): Promise<ISessionInfo> {
  ...
  // embed / standalone), keeping Contacts unchanged outside the shell.
  if (isEmbedded()) {
